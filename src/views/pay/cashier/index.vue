<!--
Template Name: Cashier m
Create author: qinglong
Create Time  : 2020-09-15
-->
<template>
  <div :class="['item',isUA=='web'?'web':'m']">
    <div class="head">
      <div class="nav"> </div>
    </div>
    <div class="body">
  ...
  methods: {
    async getData() {
      ...
      if (userinfo) {
        ...
        let { data } = await this.axios("/client/Order/orders", {
          data: { uid: userinfo.id, mobile: userinfo.mobile },
        });
        if (data.code) {
    ...
    async submit() {
      ...
      } else if (this.isUA == "h5") {
        // 手机浏览器调用
        this.pay((params) => {
          window.open(params.data, "_blank");
        });
