...
-->
<template>
  <div v-if="isUA == 'web'" class="web" @keypress.enter="login">
    <div class="box">
  ...
  <div v-else class="m" @keypress.enter="login">
    <div class="box">
      <div class="login-title">用户登录</div>
      <el-form :model="ruleForm" status-icon ref="login" label-width="80px">
        <el-form-item label="手机号码" prop="mobile" :rules="{required:true,message:'请输入账号'}">
...
export default {
  ...
  methods: {
    async getCode() {
      this.axios
        .get("/client/Login/verify?v=" + Math.random(), {
    ...
    async login() {
      let { data } = await this.axios("/client/login/login", {
        data: this.ruleForm,
      });

      if (data.code) {
        data.data.dateTime = Date.now() + 24 * 60 * 60 * 1000 * 1;
        console.log(this.storage);
        this.setstore("userinfo", data.data);
